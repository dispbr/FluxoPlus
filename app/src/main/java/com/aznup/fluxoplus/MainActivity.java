...
public class MainActivity extends Activity {
    ...
    private static final String APP_VERSION="1.1.10";
    private SharedPreferences prefs;
    private boolean biometricPromptOpen=false, pdfBoxInitialized=false, pdfChooserActive=false;
    ...
    @Override protected void onCreate(Bundle savedInstanceState){
        ...
        webView.setWebChromeClient(new WebChromeClient(){
            @Override public boolean onShowFileChooser(WebView view,ValueCallback<Uri[]> callback,FileChooserParams params){
                ...
                pdfChooserActive=false;
                try{
                    String[] types=params.getAcceptTypes();
                    if(types!=null)for(String t:types){if(t!=null&&t.toLowerCase().contains("pdf")){pdfChooserActive=true;break;}}
                    Intent i=params.createIntent();
                    startActivityForResult(i,FILE_CHOOSER);
                    return true;
                }catch(Exception e){fileCallback=null;pdfChooserActive=false;return false;}
            }
        });
    ...
    private void injectStableUi(){
        ...
        "var V='1.1.10';var av=document.getElementById('appVersion');if(av)av.textContent='Versão '+V;var sv=document.getElementById('settingsVersion');if(sv)sv.textContent=V;"+
        ...
        "window.pickPdf=function(){try{var bank=document.getElementById('pdfBank'),p=document.getElementById('pdfStatus'),a=document.getElementById('pdfAnalysis');if(p)p.textContent='Abrindo seletor de arquivos...';if(a)a.innerHTML='';var inp=document.getElementById('nativePdfInput');if(!inp){inp=document.createElement('input');inp.id='nativePdfInput';inp.type='file';inp.accept='application/pdf,.pdf';inp.style.display='none';document.body.appendChild(inp);}if(window.AndroidBridge&&AndroidBridge.setPdfBank)AndroidBridge.setPdfBank(bank?bank.value:'outro');inp.value='';inp.click();}catch(e){var p=document.getElementById('pdfStatus');if(p)p.textContent='Não foi possível abrir os arquivos.';}};"+
    ...
    public class AndroidBridge{
        ...
        @JavascriptInterface public void setPdfBank(String bank){pendingPdfBank=(bank==null||bank.trim().isEmpty())?"outro":bank.trim();}
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){
        ...
        if(requestCode==FILE_CHOOSER&&fileCallback!=null){
            ...
            if(pdfChooserActive){
                ...
                }else sendPdfError("Seleção de PDF cancelada.");
            }
            fileCallback.onReceiveValue(result);fileCallback=null;pdfChooserActive=false;
        }
    }
