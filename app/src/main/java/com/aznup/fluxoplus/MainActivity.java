...
public class MainActivity extends Activity {
    ...
    private static final String APP_VERSION="1.1.9";
    ...
    private void injectStableUi(){
        ...
        "var V='1.1.9';var av=document.getElementById('appVersion');if(av)av.textContent='Versão '+V;var sv=document.getElementById('settingsVersion');if(sv)sv.textContent=V;"+
        ...
        "window.pickPdf=function(){try{var bank=document.getElementById('pdfBank'),p=document.getElementById('pdfStatus'),a=document.getElementById('pdfAnalysis');if(p)p.textContent='Abrindo arquivos do aparelho...';if(a)a.innerHTML='';if(window.AndroidBridge&&AndroidBridge.pickCreditCardPdf)AndroidBridge.pickCreditCardPdf(bank?bank.value:'outro');else if(p)p.textContent='Importação PDF indisponível.';}catch(e){var p=document.getElementById('pdfStatus');if(p)p.textContent='Não foi possível abrir os arquivos.';}};"+
    ...
    private void openPdfPicker(String bank){
        ...
        try{
            startActivityForResult(Intent.createChooser(open,"Selecionar fatura em PDF"),PDF_CHOOSER);
            return;
        ...
        get.setType("application/pdf");
        get.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
        try{
            startActivityForResult(Intent.createChooser(get,"Selecionar fatura em PDF"),PDF_CHOOSER);
            return;
        }catch(ActivityNotFoundException ignored){}catch(Exception ignored){}
        Intent any=new Intent(Intent.ACTION_GET_CONTENT);
        any.addCategory(Intent.CATEGORY_OPENABLE);
        any.setType("*/*");
        any.putExtra(Intent.EXTRA_MIME_TYPES,new String[]{"application/pdf"});
        any.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
        try{startActivityForResult(Intent.createChooser(any,"Selecionar fatura em PDF"),PDF_CHOOSER);}
        catch(Exception e){sendPdfError("Nenhum seletor de arquivos compatível foi encontrado no aparelho.");}
    }
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){super.onActivityResult(requestCode,resultCode,data);if(requestCode==PDF_CHOOSER){if(resultCode==RESULT_OK&&data!=null&&data.getData()!=null){Uri uri=data.getData();try{int flags=data.getFlags()&Intent.FLAG_GRANT_READ_URI_PERMISSION;if(flags!=0)getContentResolver().takePersistableUriPermission(uri,flags);}catch(Exception ignored){}readPdf(uri,pendingPdfBank);}else sendPdfError("Seleção de PDF cancelada.");return;}if(requestCode==FILE_CHOOSER&&fileCallback!=null){fileCallback.onReceiveValue(WebChromeClient.FileChooserParams.parseResult(resultCode,data));fileCallback=null;}}
